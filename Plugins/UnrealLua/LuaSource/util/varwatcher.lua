...
function LuaVarWatcher:UpdateNodesChildren(UNode, VarValue, bIsKey)
	...
	if classname == "(UVarNode)" then
		return
	elseif type(VarValue) == "userdata" and (classname == "(ULuaArrayHelper)" or classname == "(ULuaSetHelper)" or classname == "(ULuaMapHelper)") then
		VarValue = VarValue:Table()
	elseif type(VarValue) == "userdata" and (string.find(classname, "^%(TArray_") or string.find(classname, "^%(TMap_") or string.find(classname, "^%(TShared")) then
		VarValue = VarValue:Table()
	end
	if UNode:bIsExpanding() or (UNode.HasExpand and not UNode:HasChild()) then
		local function TravelVar(ChildrenMap, key, v, KeyName, bTravelKey)
			...
			if classname == "(UVarNode)" then
				return
			elseif type(v) == "userdata" and (classname == "(ULuaArrayHelper)" or classname == "(ULuaSetHelper)" or classname == "(ULuaMapHelper)") then
				v = v:Table()
			end
		...
		elseif type(VarValue) == "userdata" then
			...
			if meta then
				...
				if meta.GetAllProperty then
					local AllProperty = VarValue:GetAllProperty()
					for key, v in pairs(AllProperty) do
						...
						KeyName=KeyName..self:GetClassName(key)
						TravelVar(ChildrenMap, key, v, KeyName, false)
					end
				end
...
function LuaVarWatcher:Tick( )
	...
	if UVarNode.NeedUpdate(TheGameInstance) then
...
LuaVarWatcher.Add = LuaVarWatcher.AddNode
