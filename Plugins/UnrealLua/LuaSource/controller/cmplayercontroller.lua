...
function CMPlayerController:TryCoroutine_Imp()
	local a, b = FVector, FVector_nogc
	...
	CoroutineUtil.MoveToLocation(AIController, TargetPos, -1, true, true, false, true, nil, true)
	CoroutineUtil.Delay(1)
	CoroutineUtil.MoveToLocation(AIController, NowPos, -1, true, true, false, true, nil, true)
	CoroutineUtil.MoveToLocation(AIController, NowPos, -1, true, true, false, true, nil, true)
	CoroutineUtil.Delay(1)
...
function CMPlayerController:InputTap_Release(Pos, HoldTime)
	if not self.m_bHasMoveScreen then
		...
		if self:GetHitResult(Pos[1], Pos[2], Hit, ECollisionEnabled.QueryOnly, true) then
...
function CMPlayerController:Visible(character)
	...
	if self.PlayCharacter == character then
		...
		local Hit = FHitResult.Temp()
		if UKismetSystemLibrary.LineTraceSingle(self.PlayCharacter, StartPos, EndPos, ETraceTypeQuery.TraceTypeQuery1, true, {}, EDrawDebugTrace.None, Hit, true, FLinearColor.Red, FLinearColor.Green, 5.0) then
...
function CMPlayerController:ReceiveBeginPlay( )
end

function CMPlayerController:TestBpImp( )
end
