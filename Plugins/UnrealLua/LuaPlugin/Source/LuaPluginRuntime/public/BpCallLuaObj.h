...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "LuaClass", meta = (DisplayName = "LuaInject"))
		static void _LuaInject(UObject* BPIns, const FString& LuaClassPath);

	UFUNCTION(BlueprintCallable, Category = "LuaClass", meta = (DisplayName = "LuaCtorInject"))
		static void _LuaCtorInject(UObject* BPIns, const FString& LuaClassPath);
...
// 		lua_State* L = UTableUtil::GetRunningState();
		lua_State* L = UUnrealLuaInterface::GetLuaStateByIns(Stack.Object);
		lua_pushcfunction(L, ErrHandleFunc);
		if (Z_Param_Ins == nullptr)
		{
			lua_getglobal(L, TCHAR_TO_UTF8(*Z_Param_FunctionName));
#if LuaDebug
			ensureAlwaysMsgf(!lua_isnil(L, -1), TEXT("Func doesn't exist"));
#endif
		...
		{
#if LuaDebug
			ensureAlwaysMsgf(UTableUtil::existluains(L, Z_Param_Ins), TEXT("lua ins doesn't exist"));
#endif
		...
		if (lua_gettop(L) < (ReturnCount+1))
			ensureAlwaysMsgf(0, TEXT("some bug?May be lua didn't return enough value"));
#endif
	...
	UFUNCTION(BlueprintCallable, CustomThunk, Category = "LuaClass", meta = (DisplayName = "LuaCall_0_0"))
		static void Call(UObject* Ins, const FString& FunctionName);
	DECLARE_FUNCTION(execCall)
	{
		push_and_call(0, 0, Stack);
	}
